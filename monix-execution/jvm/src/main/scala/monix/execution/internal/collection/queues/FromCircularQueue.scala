...
private[internal] object FromCircularQueue {
  ...
  private final class FencedSPMC[A](queue: MessagePassingQueue[A]) extends FromCircularQueue[A](queue) {
    def fenceOffer(): Unit = VarHandle.fullFence()
    def fencePoll(): Unit = ()
  }

  private final class FencedMPSC[A](queue: MessagePassingQueue[A]) extends FromCircularQueue[A](queue) {
    def fenceOffer(): Unit = ()
    def fencePoll(): Unit = VarHandle.fullFence()
  }

  private final class FencedSPSC[A](queue: MessagePassingQueue[A]) extends FromCircularQueue[A](queue) {
    def fenceOffer(): Unit = VarHandle.fullFence()
    def fencePoll(): Unit = VarHandle.fullFence()
  }
}
